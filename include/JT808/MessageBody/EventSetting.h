...
public:
    ...
    EventSetting(SettingTypes type, const std::vector<Event>& events);
    void parse(const std::vector<uint8_t>& data) override;
    void parse(const uint8_t* data, int size) override;
    std::vector<uint8_t> package() override;
    bool operator==(const EventSetting& other);

    SettingTypes type() const;
    void setType(SettingTypes newType);
...
private:
    SettingTypes m_type = DeleteAllEvents;
    uint8_t m_length = 0;
