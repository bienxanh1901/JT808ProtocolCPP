...
protected:
    void SetUp() override
    {
        ...
        m_object = nlohmann::json::object(
            {{"type", InformationServiceMenuSetting::AppendMenu},
             {"length", 2},
             {"menus",
              nlohmann::json::array({nlohmann::json::object({{"type", 123}, {"info", "Hello World"}}),
                                     nlohmann::json::object({{"type", 123}, {"info", "JT808"}})})}});
    }
};
